...
def CPC15_BEASTsimulation(DistA, DistB, Amb, Corr,p1,p2,p3,p4,p5,original):
    ...
    for trial in range(nTrials):
        ...
        for s in range(1, kapa[0]+1):
            ...
            if (rndNum[0]<pBias2[block][0] and not original) or (rndNum[0] > pBias[nfeed] and original):# Unbiased technique
                if nfeed == 0:
            ...
            elif (rndNum[0]<pBias2[block][1] and not original) or (rndNum[0] > (2 / 3) * pBias[nfeed] and original):  # uniform
                outcomeA = distSample(DistA[:, 0], np.repeat([1 / nA], nA), rndNum[1])
                outcomeB = distSample(DistB[:, 0], np.repeat([1 / nB], nB), rndNum[1])
            elif (rndNum[0] < pBias2[block][2] and not original) or (rndNum[0] > (1 / 3) * pBias[nfeed] and original):  # contingent pessimism
