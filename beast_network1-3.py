...
logging.basicConfig(filename='/data/home/vhalpern/pythonProject1/TEST_TRAIN/beast_network1-3.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([1.6365,  2.0702, -0.4683,  1.7615], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([ 2.5913,  1.8325, -0.2347,  1.5109], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.7317,  1.7759, -0.1907,  1.4830], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([2.8725,  1.7185, -0.1447,  1.4536], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([3.0121,  1.6615, -0.0972,  1.4235], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(-0.0173, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.7908, dtype=torch.float64))
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        pd.read_csv('c13k_selections.csv').merge(df2[['BEASTNET','Problem']],on='Problem').to_csv('TEST_TRAIN/c13k_selections1-3.csv',index=False)
