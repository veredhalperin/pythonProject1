...
logging.basicConfig(filename='/data/home/vhalpern/pythonProject1/TEST_TRAIN7/beast_network_cluster3.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([1.7954,  1.6531, -0.2993,  1.8508], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([ 2.7365,  1.5131, -0.0646,  1.5150], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.8622,  1.4833, -0.0235,  1.4780], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([2.9866, 1.4528, 0.0187, 1.4420], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([3.1092, 1.4221, 0.0615, 1.4072], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(0.1907, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.4067, dtype=torch.float64))
...
if __name__ == '__main__':
   ...
   #"""
    ...
    df=pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN7/c13k_selections_clusters.csv')
    df = df[df['cluster'] == 3]
    ...
    with torch.no_grad():
        ...
        pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN7/c13k_selections_clusters.csv').merge(df2[['BEASTNET','Problem']],on='Problem').to_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN7/c13k_selections_cluster3.csv',index=False)
