...
class ProbsNet(torch.nn.Module):
    ...
    def forward(self, bv,bp,BEVa, ST0, Weight0, ST1, Weight1):
        ...
        for i in range(len(Weight0)):
            tmp0 = torch.cat((tmp0, torch.matmul(1 / (1 + torch.exp(-self.B * (torch.nn.functional.relu(self.BEV) * (torch.matmul(bp,bv)-BEVa) + (ST0[i])))), Weight0[i])))
        for i in range(len(Weight1)):
            tmp1 = torch.cat((tmp1, torch.matmul(1 / (1 + torch.exp(-self.B * (torch.nn.functional.relu(self.BEV) * (torch.matmul(bp,bv)-BEVa) + (ST1[i])))), Weight1[i])))
