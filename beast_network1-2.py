...
logging.basicConfig(filename='/data/home/vhalpern/pythonProject1/TEST_TRAIN/beast_network1-2.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([1.6480,  2.0280, -0.4353,  1.7593], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([2.5916,  1.8589, -0.2020,  1.4514], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.7269,  1.8093, -0.1584,  1.4222], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([2.8629,  1.7568, -0.1130,  1.3932], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([2.9984,  1.7029, -0.0663,  1.3649], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(0.2926, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.4282, dtype=torch.float64))
...
if __name__ == '__main__':
   ...
   """
    ...
    df = df[df['Dom'] == 0]
    df = df[((df['av2'].isna())&(df['bv3'].isna()))|(df['bv2'].isna())]#1-2
    ...
    with torch.no_grad():
        ...
        pd.read_csv('c13k_selections.csv').merge(df2[['BEASTNET','Problem']],on='Problem').to_csv('TEST_TRAIN/c13k_selections1-2.csv',index=False)
