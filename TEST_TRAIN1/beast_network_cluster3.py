...
logging.basicConfig(filename='/data/home/vhalpern/pythonProject1/TEST_TRAIN1/beast_network_cluster3.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([1.7697,  1.6487, -0.2722,  1.8537], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([ 2.7121,  1.5223, -0.0426,  1.5083], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.8360e+00,  1.4940e+00, -2.3383e-03,  1.4723e+00], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([2.9591, 1.4647, 0.0390, 1.4372], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([3.0807, 1.4349, 0.0810, 1.4034], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(0.1828, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.3438, dtype=torch.float64))
...
if __name__ == '__main__':
   ...
   #"""
    ...
    df=pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN1/c13k_selections_clusters.csv')
    df = df[df['cluster'] == 3]
    ...
    with torch.no_grad():
        ...
        pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN1/c13k_selections_clusters.csv').merge(df2[['BEASTNET','Problem']],on='Problem').to_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN1/c13k_selections_cluster3.csv',index=False)
