...
logging.basicConfig(filename='/data/home/vhalpern/pythonProject1/TEST_TRAIN0/beast_network_cluster3.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([1.7620, 1.6216, -0.2747, 1.8912], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([ 2.7099, 1.5109, -0.0445, 1.5237], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.8361, 1.4828, -0.0040, 1.4852], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([2.9611, 1.4535, 0.0375, 1.4480], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([3.0843, 1.4237, 0.0797, 1.4123], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(0.1580, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.3638, dtype=torch.float64))
...
if __name__ == '__main__':
   ...
   #"""
    ...
    df=pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN0/c13k_selections_clusters.csv')
    df = df[df['cluster'] == 3]
    ...
    with torch.no_grad():
        ...
        pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN0/c13k_selections_clusters.csv').merge(df2[['BEASTNET','Problem']],on='Problem').to_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN0/c13k_selections_cluster3.csv',index=False)
