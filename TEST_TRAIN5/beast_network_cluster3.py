...
logging.basicConfig(filename='/data/home/vhalpern/pythonProject1/TEST_TRAIN5/beast_network_cluster3.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([1.7324,  1.6631, -0.3075,  1.9120], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([ 2.6895,  1.5504, -0.0790,  1.5391], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.8182,  1.5202, -0.0382,  1.4997], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([2.9459, 1.4887, 0.0040, 1.4613], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([3.0719, 1.4565, 0.0471, 1.4244], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(0.1576, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.4073, dtype=torch.float64))
...
if __name__ == '__main__':
   ...
   #"""
    ...
    df=pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN5/c13k_selections_clusters.csv')
    df = df[df['cluster'] == 3]
    ...
    with torch.no_grad():
        ...
        pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN5/c13k_selections_clusters.csv').merge(df2[['BEASTNET','Problem']],on='Problem').to_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN5/c13k_selections_cluster3.csv',index=False)
