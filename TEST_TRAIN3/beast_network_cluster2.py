...
logging.basicConfig(filename='/data/home/vhalpern/pythonProject1/TEST_TRAIN3/beast_network_cluster2.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([1.7696,  1.6358, -0.2848,  1.8793], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([ 2.7143,  1.5222, -0.0521,  1.5156], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.8381,  1.4947, -0.0112,  1.4785], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([2.9610, 1.4659, 0.0307, 1.4425], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([3.0824, 1.4364, 0.0733, 1.4078], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(0.2126, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.3377, dtype=torch.float64))
...
if __name__ == '__main__':
   ...
   #"""
    ...
    df=pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN3/c13k_selections_clusters.csv')
    df = df[df['cluster'] == 2]
    ...
    with torch.no_grad():
        ...
        pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN3/c13k_selections_clusters.csv').merge(df2[['BEASTNET','Problem']],on='Problem').to_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN3/c13k_selections_cluster2.csv',index=False)
