...
logging.basicConfig(filename='/data/home/vhalpern/pythonProject1/TEST_TRAIN/beast_network2-3.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([1.7585,  1.3281, -0.3191,  2.2324], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([ 2.8240,  1.3160, -0.0912,  1.6512], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.9513,  1.3032, -0.0505,  1.5960], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([3.0756,  1.2889, -0.0084,  1.5438], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([3.1967, 1.2736, 0.0347, 1.4949], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(0.1332, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.2767, dtype=torch.float64))
...
if __name__ == '__main__':
   ...
   """
    ...
    df = df[df['Dom'] == 0]
    df = df[(~df['av2'].isna()) & (~df['bv3'].isna())]  # 2-3
    ...
    with torch.no_grad():
        ...
        pd.read_csv('c13k_selections.csv').merge(df2[['BEASTNET','Problem']],on='Problem').to_csv('TEST_TRAIN/c13k_selections2-3.csv',index=False)
