...
logging.basicConfig(filename='/data/home/vhalpern/pythonProject1/TEST_TRAIN/beast_network_dom.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([1.3845, 1.1202, 1.2274, 1.2678], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([ 2.0401, 1.1735, 1.2664, 1.2200], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.1434, 1.1778, 1.2674, 1.2114], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([2.2501, 1.1807, 1.2668, 1.2024], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([2.3603, 1.1821, 1.2644, 1.1932], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(-0.0022, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.8218, dtype=torch.float64))
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        pd.read_csv('c13k_selections.csv').merge(df2[['BEASTNET','Problem']],on='Problem').to_csv('TEST_TRAIN/c13k_selections_dom.csv',index=False)
