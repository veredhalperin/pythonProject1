...
logging.basicConfig(filename='beast_network8.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        #self.pBEV= torch.nn.Parameter(torch.tensor(0.1, dtype=torch.float64))
        #self.pST = torch.nn.Parameter(torch.tensor(0.1, dtype=torch.float64))

    def forward(self, bv,bp,BEVa, ST0, Weight0, ST1, Weight1):
        ...
        for i in range(len(Weight0)):
            tmp0 = torch.cat((tmp0, torch.matmul(1 / (1 + torch.exp(-self.B * (torch.nn.functional.relu(self.BEV) * (torch.matmul(torch.nn.functional.softmax(bp/(self.t1+eps)),bv)-BEVa) + (ST0[i])))), Weight0[i])))
        for i in range(len(Weight1)):
            tmp1 = torch.cat((tmp1, torch.matmul(1 / (1 + torch.exp(-self.B * (torch.nn.functional.relu(self.BEV) * (torch.matmul(torch.nn.functional.softmax(bp/(self.t2+eps)),bv)-BEVa) + (ST1[i])))), Weight1[i])))
...
if __name__ == '__main__':
    ...
    params = ['probs0', 'probs1', 'probs2', 'probs3', 'probs4', 'BEV', 'B','t1','t2']
    ...
    df = df[(df['av2'].isna()) & (~df['bv3'].isna())]#1-3
    df=df.fillna(0)
            ...
            if df2['TEST_TRAIN'][i]=='TRAIN':
                ...
                Pred = torch.cat(
                    (Pred, torch.reshape(model(
                        torch.reshape(torch.tensor([df2['bv'+str(j)][i] for j in range(1,11)],dtype=torch.float64),(10,1)),
                        torch.reshape(torch.tensor([df2['bp'+str(j)][i] for j in range(1,11)], dtype=torch.float64),(1,10)),
                        df2['BEVa'][i],
    ...
    with torch.no_grad():
        ...
        df2['NEURAL_BEAST2']=df2.apply(lambda x:(model(
            torch.reshape(torch.tensor([df2['bv'+str(j)][i] for j in range(1,11)],dtype=torch.float64),(10,1)),
            torch.reshape(torch.tensor([df2['bp'+str(j)][i] for j in range(1,11)], dtype=torch.float64),(1,10)),
            df2['BEVa'][i],
        ...
        pd.read_csv('c13k_selections-1.csv').merge(df2[['NEURAL_BEAST2','Problem']],on='Problem').to_csv('c13k_selections8.csv',index=False)
        print(datetime.now())
        logging.debug(datetime.now())