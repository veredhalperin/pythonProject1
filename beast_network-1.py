...
logging.basicConfig(filename='beast_network-1.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([2.1, 1.0, 1.0, 1.0], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([2.7, 1.0, 1.0, 1.0], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.85, 1.0, 1.0, 1.0], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([2.95, 1.0, 1.0, 1.0], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([3.05, 1.0, 1.0, 1.0], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(0.3, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.2, dtype=torch.float64))
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        df2['NEURAL_BEAST']=df2.apply(lambda x:(model(x['BEVb'] - x['BEVa'],
        ...
        pd.read_csv('c13k_selections.csv').merge(df2[['NEURAL_BEAST','Problem']],on='Problem').to_csv('c13k_selections-1.csv',index=False)
        print(datetime.now())
        logging.debug(datetime.now())