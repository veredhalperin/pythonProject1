...
logging.basicConfig(filename='/data/home/vhalpern/pythonProject1/TEST_TRAIN8/beast_network_cluster2.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([1.7881,  1.6221, -0.2364,  1.8261], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([ 2.7186e+00,  1.5052e+00, -1.5984e-03,  1.4778e+00], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.8409, 1.4774, 0.0382, 1.4435], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([2.9621, 1.4487, 0.0789, 1.4103], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([3.0819, 1.4196, 0.1201, 1.3784], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(0.2063, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.3036, dtype=torch.float64))
...
if __name__ == '__main__':
   ...
   #"""
    ...
    df=pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN8/c13k_selections_clusters.csv')
    df = df[df['cluster'] == 2]
    ...
    with torch.no_grad():
        ...
        pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN8/c13k_selections_clusters.csv').merge(df2[['BEASTNET','Problem']],on='Problem').to_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN8/c13k_selections_cluster2.csv',index=False)
