...
class SET_MLP_CIFAR10:
    def __init__(self):
        ...
        [self.noPar1, self.wm1] = createWeightsMask(self.epsilon,18, 4000)
        ...
        Data = Data[Data['Dom'] == 0]
        i=0
        test = Data[(Data['Problem'] > i) & (Data['Problem'] <= i + 1300)]
        print("total loss",keras.losses.mean_squared_error(np.array(test['bRate']), self.model.predict(np.array(test[['Ha', 'pHa', 'La', 'lot_shape__A',
                                                                                    'lot_shape_symm_A', 'lot_shape_L_A',
                                                                                    'lot_shape_R_A', 'LotNumA', 'Hb',
                                                                                    'pHb', 'Lb', 'lot_shape__B',
                                                                                    'lot_shape_symm_B', 'lot_shape_L_B',
                                                                                    'lot_shape_R_B', 'LotNumB', 'Amb',
                                                                                    'Corr']]))))
    ...
    def create_model(self):
        ...
        self.model.add(Dropout(0.3))
        self.model.add(Dense(1, name="dense_4",weights=self.w4)) #please note that there is no need for a sparse output layer as the number of classes is much smaller than the number of input hidden neurons
    ...
    def read_data(self):
        ...
        #read CIFAR10 data
        train = pd.read_csv('GamesDom.csv')
        Data = pd.read_csv('c13k_selections-1.csv')
        Data = Data[Data['Dom'] == 0]
        i=0
        test = Data[(Data['Problem'] <= i) | (Data['Problem'] > i + 1300)]
        features = ['Ha', 'pHa', 'La', 'lot_shape__A', 'lot_shape_symm_A', 'lot_shape_L_A', 'lot_shape_R_A', 'LotNumA',
