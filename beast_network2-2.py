...
logging.basicConfig(filename='/data/home/vhalpern/pythonProject1/TEST_TRAIN/beast_network2-2.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([1.6700,  1.4217, -0.3061,  2.2145], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([ 2.6760,  1.4175, -0.0709,  1.6774], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.8073,  1.4004, -0.0300,  1.6222], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([2.9370, 1.3813, 0.0123, 1.5693], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([3.0645, 1.3605, 0.0553, 1.5195], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(0.2751, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.2420, dtype=torch.float64))
...
if __name__ == '__main__':
   ...
   """
    ...
    df = df[df['Dom'] == 0]
    df = df[(~df['av2'].isna()) & (~df['bv2'].isna()) & (df['bv3'].isna())]  # 2-2
    ...
    with torch.no_grad():
        ...
        pd.read_csv('c13k_selections.csv').merge(df2[['BEASTNET','Problem']],on='Problem').to_csv('TEST_TRAIN/c13k_selections2-2.csv',index=False)
