...
logging.basicConfig(filename='/data/home/vhalpern/pythonProject1/TEST_TRAIN6/beast_network_cluster0.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([1.7509,  1.6668, -0.3129,  1.8952], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([ 2.7347,  1.5275, -0.0793,  1.5171], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.8638,  1.4962, -0.0379,  1.4780], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([2.9912, 1.4640, 0.0046, 1.4402], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([3.1163, 1.4317, 0.0478, 1.4041], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(0.1800, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.3760, dtype=torch.float64))
...
if __name__ == '__main__':
   ...
   #"""
    ...
    df=pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN6/c13k_selections_clusters.csv')
    df = df[df['cluster'] == 0]
    ...
    with torch.no_grad():
        ...
        pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN6/c13k_selections_clusters.csv').merge(df2[['BEASTNET','Problem']],on='Problem').to_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN6/c13k_selections_cluster0.csv',index=False)
