...
logging.basicConfig(filename='beast_network5.log', level=logging.DEBUG)
import pandas as pd
...
if __name__ == '__main__':
    ...
    df = df[(df['LotShapeB'] == 'L-skew') & (df['Dom'] == 0)]
    ...
    with torch.no_grad():
        ...
        pd.read_csv('c13k_selections-1.csv').merge(df2[['NEURAL_BEAST2','Problem']],on='Problem').to_csv('c13k_selections5.csv',index=False)
        print(datetime.now())
        logging.debug(datetime.now())