...
logging.basicConfig(filename='/data/home/vhalpern/pythonProject1/TEST_TRAIN4/beast_network_cluster1.log', level=logging.DEBUG)
import pandas as pd
...
class ProbsNet(torch.nn.Module):
    def __init__(self):
        ...
        self.probs0 = torch.nn.Parameter(torch.tensor([1.7494,  1.6178, -0.2575,  1.8903], dtype=torch.float64))
        self.probs1 = torch.nn.Parameter(torch.tensor([ 2.7174,  1.4976, -0.0243,  1.5094], dtype=torch.float64))
        self.probs2 = torch.nn.Parameter(torch.tensor([2.8437, 1.4691, 0.0162, 1.4710], dtype=torch.float64))
        self.probs3 = torch.nn.Parameter(torch.tensor([2.9686, 1.4397, 0.0576, 1.4341], dtype=torch.float64))
        self.probs4 = torch.nn.Parameter(torch.tensor([3.0915, 1.4101, 0.0996, 1.3988], dtype=torch.float64))
        self.BEV = torch.nn.Parameter(torch.tensor(0.1636, dtype=torch.float64))
        self.B = torch.nn.Parameter(torch.tensor(0.3668, dtype=torch.float64))
...
if __name__ == '__main__':
   ...
   #"""
    ...
    df=pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN4/c13k_selections_clusters.csv')
    df = df[df['cluster'] == 1]
    ...
    with torch.no_grad():
        ...
        pd.read_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN4/c13k_selections_clusters.csv').merge(df2[['BEASTNET','Problem']],on='Problem').to_csv('/data/home/vhalpern/pythonProject1/TEST_TRAIN4/c13k_selections_cluster1.csv',index=False)
